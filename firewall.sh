...
##### Permanently edited this -> echo "1" > /proc/sys/net/ipv4/ip_forward
...
$IPTABLES -A FORWARD -p icmp -d 8.8.8.8 -s $Client_NET_IP -j ACCEPT -m comment --comment "allow icmp forwarding to 8.8.8.8"

#  4.3 INPUT chain
#   Provide your input rules below to allow web traffic from client
$IPTABLES -A INPUT -p tcp -d $NAT_WEB_IP_ADDRESS -s $Client_NET_IP -j ACCEPT -m comment --comment "allow client to access webpage"

#  4.3 OUTPUT chain
#   Provide your output rules below to allow web traffic (port 80) go back to client
$IPTABLES -A OUTPUT -p tcp -s $NAT_WEB_IP_ADDRESS -d $Client_NET_IP -j ACCEPT -m comment --comment "allow client access to webpage"
                                                                  
# 5. NAT setup                                                                   
#  5.1  PREROUTING chain. (Not used in this lab)
...
#   Provide your NAT POSTROUTING rules (packets go to the internet domain)
#   Add your own rule below to only allow ping from client to 8.8.8.8 on internet
$IPTABLES -t nat -A POSTROUTING -o $Internet_IFACE -s $Client_NET_IP -j MASQUERADE -m comment --comment "allow client to access 8.8.8.8 with a different IP"
